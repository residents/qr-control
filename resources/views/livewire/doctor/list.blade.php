				    ...
				    <tbody>
				    	@forelse($doctors as $doctor)
				    		<tr>
				    			...
				    			<td class="text-center">{{$doctor->document}}</td>
				    			<td class="text-center">{{$doctor->specialty->name}}</td>
				    			<td class="text-center">{{$doctor->created_at}}</td>
				    			<td>
				    				<button type="button" class="btn mx-2" wire:click="edit({{$doctor->id}})"><i class="fa fa-edit"></i> </button>
				    				<button type="button" class="btn mx-2" wire:click="$emit('triggerDelete', {{$doctor->id}})"><i class="fa fa-trash"></i> </button>
				    			</td>
				    		</tr>
				    	@empty
				            <tr>
                            ...
                            <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
	                            <div class="flex flex-wrap -mx-3 mb-6">
	                        		<div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
	                                	...
	                                	<input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="nameInput" placeholder="Nombre del médico" wire:model="name">
	                                	@error('name') <span class="text-red-500">{{ $message }}</span>@enderror
	                                </div>
	                            </div>
	                            <div class="flex flex-wrap -mx-3 mb-6">
	                                <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
	                                	<label for="documentInput" class="block text-gray-700 text-sm font-bold mb-2">Cédula:</label>
	                                	<input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="documentInput" placeholder="Cédula profesional" wire:model="document">
	                                	@error('document') <span class="text-red-500">{{ $message }}</span>@enderror
	                                </div>
	                            </div>
	                            <div class="flex flex-wrap -mx-3 mb-6">
	                                <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
	                                	<label for="specialtyInput" class="block text-gray-700 text-sm font-bold mb-2">Especialidad:</label>
	                                	<select type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="specialtyInput" placeholder="Especialidad" wire:model="id_specialty">
	                                		<option value="" selected="">Seleccione una especialidad</option>
	                                		@forelse($specialties as $specialty)
	                                		<option value="{{$specialty->id}}">{{$specialty->name}}</option>
	                                		@empty
	                                			<option value="" disabled="">No se han registrado especialidades</option>
	                                		@endforelse
	                                	</select>
	                                	@error('id_specialty') <span class="text-red-500">{{ $message }}</span>@enderror
	                                </div>
	                            </div>
                            </div>
...
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        @this.on('triggerDelete', companyId => {
            Swal.fire({
                title: '¿Está seguro?',
                text: 'El registro del medico será borrado , al igual que las citas programadas con el.',
                type: "warning",
